...
CarouselImage::CarouselImage(QWidget *parent) : QWidget(parent), m_currentDrawImageIndx(0) {
    // 添加ImageOpacity属性
    ...
    // 设置ImageOpacity属性值变化范围
    m_opacityAnimation->setStartValue(1.0);
    m_opacityAnimation->setEndValue(0.0);
    // 透明度变化时更新绘图
    connect(m_opacityAnimation, SIGNAL(valueChanged(
    const QVariant&)), this, SLOT(update()));
    // 图片切换时钟槽函数
...
void CarouselImage::initChangeImageButton() {
    // 图片过多 按钮可能放置不下
    ...
    for (int i = 0; i < m_imageFileNameList.count(); i++) {
        ...
        pButton->setStyleSheet("QPushButton{border-image:url(:image/coo1);}\
                                QPushButton:checked{border-image:url(:image/coo2);}");
...
void CarouselImage::setImageList(QStringList imageFileNameList) {
    m_imageFileNameList = imageFileNameList;
}

void CarouselImage::addImage(QString imageFileName) {
    m_imageFileNameList.append(imageFileName);
}

void CarouselImage::startPlay() {
...
void CarouselImage::onImageChangeTimeout() {
    // 设置前后的图片
    m_currentPixmap = QPixmap(m_imageFileNameList.at(m_currentDrawImageIndx));
...
void CarouselImage::paintEvent(QPaintEvent *event) {
...
void CarouselImage::onImageSwitchButtonClicked(int buttonId) {
...
void CarouselImage::mousePressEvent(QMouseEvent *event) {
    // 当前图片点击触发函数
